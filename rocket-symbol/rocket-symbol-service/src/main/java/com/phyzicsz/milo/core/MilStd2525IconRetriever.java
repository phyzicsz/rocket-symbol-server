 ...
 * tactical symbols. This hierarchy is implemented using retrieval parameters
 * SHOW_FILL, SHOW_FRAME, and SHOW_ICON. By default, all three elements are
 * displayed, and they can be turned off by setting the appropriate parameter.
 * If frame and icon are turned off the retriever will return an image that
 ...
 * <tr><td>SymbologyConstants.SHOW_ICON</td><td>Boolean</td><td>Determines if
 * the symbol will be created with an icon.</td></tr>
 * <tr><td>SymbologyConstants.SHOW_FRAME</td><td>Boolean</td><td>Determines if
 * the symbol will be created with a frame.</td></tr>
 * <tr><td>SymbologyConstants.SHOW_FILL</td><td>Boolean</td><td>Determines if
 * the symbol will be created with a fill color.</td></tr><tr><td valign="top">AVKey.COLOR</td><td
 * valign="top">java.awt.Color</td><td valign="top">Fill color applied to the
 * symbol. If the symbol is drawn with a frame, then this color will be used to
 * fill the frame. If the symbol is not drawn with a frame, then the fill will
...
public class MilStd2525IconRetriever extends AbstractIconRetriever {
    ...
    private static final Logger logger = LoggerFactory.getLogger(MilStd2525IconRetriever.class);
     ...
     * when both frame and icon are off.
     */
     ...
     */
    @Override
    public BufferedImage createIcon(String sidc, AVList params) throws IOException {
        if (sidc == null) {
    ...
    protected BufferedImage drawIcon(SymbolCode symbolCode, AVList params, BufferedImage dest) throws IOException {
    ...
    protected BufferedImage drawIconComponent(String path, Color color, BufferedImage dest) {
        ...
        if (image == null) {
            logger.error("missing icon component");
            throw new IllegalArgumentException("missing icon component");
        }
    ...
    protected String composeIconPath(SymbolCode symbolCode, AVList params) throws IOException {
     ...
     *
     * @return The value of the AVKey.COLOR parameter, if such a parameter
     * exists and is of type java.awt.Color. Returns null if the parameter list
    ...
    protected String getMaskedIconCode(SymbolCode symbolCode, AVList params) throws IOException {
        String si = this.getSimpleStandardIdentity(symbolCode); // Either Unknown, Friend, Neutral, or Hostile.
        String status = this.getSimpleStatus(symbolCode); // Either Present or Anticipated.
